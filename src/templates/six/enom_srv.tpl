{if $id}
	...
	<div class="alert alert-info" style="display:none;position:absolute;top:25px;right:30px;" id="ajax_loading">Loading
	</div>
	<form method="POST" action="{$smarty.server.PHP_SELF}" id="srv_form">
		...
		<table class="table">
			...
			{foreach from=$records item=v}
				<tr>
					<td><input type="text" name="records[{$v}][service]" class="span2" {if $v == 1}placeholder="_voice"{/if}/>
					</td>
					...
					<td>
						<button data-id="{$v}" class="btn mini delete"><i class="icon-trash"></i></button>
					</td>
				</tr>
			{/foreach}
			</tbody>
